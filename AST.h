 ...
 */
typedef class AST_node_ *AST_node;
...
public:
	...
	virtual string result_label(AST_node_ *who=0) {
		if (this->stored_result_label.compare("-1") == 0) 
		{
			this->stored_result_label = this->init_result_label();
		}
		return stored_result_label;
	}
	virtual string init_result_label();
...
// protected:  // so that derived class's set_parent should be able to get at stored_parent for "this" object ... Smalltalk allows this by default
	...
	int my_FP = 0;
	ST<var_info> myVarLibrary=ST<var_info>();

	string stored_result_label = "-1";
	
	virtual string inAssign(AST_node_ *child);
	virtual int FPstart(AST_node_ *who);
	virtual ST<var_info>* myLetVarLibrary();
	virtual Symbol closestFunction(Symbol name);
	virtual bool inBody(AST_node_ *who);

	//return how many retreive from 2,FP i need 
	//return the offset
	virtual std::tuple<int, var_info*> findVarFP(int count, Symbol s); 

	ST<sym_info> myFuncLibrary;
	virtual ST<sym_info>* myLetFuncLibrary();
...
public:
	...
	virtual string HERA_data();
	virtual Ty_ty typecheck();
	virtual void set_parent_pointers_for_me_and_my_descendants(AST_node_ *my_parent_or_null_if_i_am_the_root);
private:
	A_var _var;
	virtual int init_result_reg();
...
typedef enum {A_plusOp, A_minusOp, A_timesOp, A_divideOp, A_modOp,
	     A_eqOp, A_neqOp, A_ltOp, A_leOp, A_gtOp, A_geOp, A_notOp, A_orOp, A_andOp} A_oper;
...
class A_compExp_ : public A_exp_ {
public:
	A_compExp_(A_pos pos, A_oper oper, A_exp left, A_exp right);
	virtual string print_rep(int indent, bool with_attributes);
	virtual string HERA_code();
	virtual string HERA_data();
	virtual Ty_ty typecheck();
...
public:
	...
	virtual string HERA_code();
	virtual string HERA_data();
	virtual Ty_ty typecheck(Ty_fieldList expected=0);
...
public:
	...
	void set_parent_pointers_for_me_and_my_descendants(AST_node_ *my_parent);
	virtual int FPstart(AST_node_ *who);
	int alreadyChecked = -1;
private:
	Symbol _var;
	Symbol _typ;
	A_exp _init;
	int virtual init_result_reg();
	// Appel had this here:
	//	bool escape;
...
class A_fundec_ : public A_dec_ {  // possibly this would be happier as a subclass of "A_dec_"?
public:
	...
	ST<sym_info>* myLetFuncLib=nullptr;
	ST<var_info>* myLetVarLib=nullptr;

	virtual int FPstart(AST_node_ *who);
	void goOverFunctions();
	int firstGoOver = -1;
	void init_myVarLibrary(A_fieldList param, int count);
	virtual std::tuple<int, var_info*> findVarFP(int count, Symbol s); 
...
private:
	Symbol _name;
	A_fieldList _params;
	Ty_fieldList _params_type=0;
	Symbol _result;
	int _paramsLen=0;
	int _paramsAndResultReg=0;
	A_exp _body;
	int stored_result_reg = -1;
	virtual int init_result_reg();
};
